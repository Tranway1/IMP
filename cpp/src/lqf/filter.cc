...
namespace lqf {
    ...
    shared_ptr<Bitmap> SimplePredicate::filterBlock(Block &block, Bitmap &skip) {
      ...
      if (skip.isFull()) {
        ...
        for (uint64_t i = 0; i < block_size; ++i) {
          auto cur = ite->next();
          //                std::cout<<"value: "<< ByteArrayToString(cur.asByteArray()) <<", index:"<< i <<std::endl;
          if (predicate_(cur)) {
            //                    std::cout<<"value: "<< ByteArrayToString(cur.asByteArray()) <<", true:"<< i <<std::endl;
            result->put(ite->pos());
          }
        }
      } else {
      ...
      std::cout<<"counting: "<< result->cardinality() <<std::endl;
      return result;
    }
    template<typename DTYPE>
    SimpleDictPredicate<DTYPE>::SimpleDictPredicate(uint32_t index, char op,T &target)
        : ColPredicate(index), target_(target), op_(op) {
      std::cout<<"init pred!"<< std::endl;
    }

    template<typename DTYPE>
    void SimpleDictPredicate<DTYPE>::predicate(char op) {
      std::cout<<"set pred!"<< std::endl;
      if (op=='='){
        this->predicate_ =  [this](int x) { return x == rawTarget_;};
      }
      else if (op=='>'){
        if(rawTarget_ < 0) {
        ...
        if(rawTarget_ < 0) {
          rawTarget_ = -rawTarget_-1;
        }
        this->predicate_ =  [this](int x) { return x < rawTarget_;};
      }
      else {
        std::cout<< "operator not supported, predicate not set"<< std::endl;
      }

    }

    template<typename DTYPE>
    shared_ptr<Bitmap> SimpleDictPredicate<DTYPE>::filterBlock(Block &block, Bitmap &skip) {
      auto dictionary = static_cast<ParquetTable*>(block.owner())->LoadDictionary<DTYPE>(index_);
      rawTarget_ = dictionary->lookup(target_);
      std::cout<<"translate to: "<< rawTarget_ ;
      predicate(op_);
      std::cout<<"and then to: "<< rawTarget_ <<std::endl;
      auto result = make_shared<SimpleBitmap>(block.limit());
      std::cout<<"filterBlock skip:"<< skip.isFull()<<"; index: "<<index_<<std::endl;
      auto ite = block.col(index_);
      if (skip.isFull()) {
    ...
    template
        class SimpleDictPredicate<Int32Type>;

    template
        class SimpleDictPredicate<DoubleType>;

    template
        class SimpleDictPredicate<ByteArrayType>;
    ...
    shared_ptr<Table> ColFilter::filter(Table &input) {
      std::cout<<"ColFilter filter"<< std::endl;
    ...
    shared_ptr<Bitmap> ColFilter::filterBlock(Block &input) {
        std::cout<<"ColFilter filterBlock"<< std::endl;
    ...
    shared_ptr<Bitmap> RowFilter::filterBlock(Block &input) {
        ...
        auto rit = input.rows();
        std::cout<<"RowFilter filterBlock"<< std::endl;
    ...
    shared_ptr<Bitmap> SboostRowFilter::filterBlock(Block &block) {
        std::cout<<"SboostRowFilter filterBlock"<< std::endl;
...
//            return dynamic_cast<ParquetBlock &>(block).raw(index_, accessor.get());
        ...
        void DictEq<DTYPE>::dict(Dictionary<DTYPE> &dict) {
            ...
            std::cout<<"translate to: "<< rawTarget_ <<std::endl;
        }
        ...
        void DictEq<DTYPE>::scanPage(uint64_t numEntry, const uint8_t *data,
            ...
            std::cout<<"scanPage bitWidth: "<< bitWidth <<std::endl;
//            if(rawTarget_<0){
//              return;
//            }
        ...
        void DictLess<DTYPE>::dict(Dictionary<DTYPE> &dict) {
            ...
            if(rawTarget_ < 0) {
                rawTarget_ = -rawTarget_-1;
                std::cout<<"then "<< rawTarget_ <<std::endl;
            }
//            for (int i=1000;i<1100;i++){
//              std::cout<<i<<", "<< (dict[i - 1]<dict[i])<< std::endl;
////            }
            std::cout<<"< prevoius "<< (dict[rawTarget_ - 1] < target_) <<"cur "<< (dict[rawTarget_]<target_) <<
                "after "<< (dict[rawTarget_ + 1]<target_)<< std::endl;
            std::cout<<"= prevoius "<< (dict[rawTarget_ - 1] == target_) <<"cur "<< (dict[rawTarget_]==target_) <<
                "after "<< (dict[rawTarget_ + 1]==target_)<< std::endl;
            std::cout<<"> prevoius "<< (dict[rawTarget_ - 1] > target_) <<"cur "<< (dict[rawTarget_]>target_) <<
                "after "<< (dict[rawTarget_ + 1]>target_)<< std::endl;
