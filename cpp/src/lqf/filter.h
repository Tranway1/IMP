...
namespace lqf {
    ...
    class SimpleDictPredicate : public ColPredicate {
      using T = typename DTYPE::c_type;
     private:
      const T &target_;
      int rawTarget_;
      char op_;
      function<bool(const int &)> predicate_;
     public:
      SimpleDictPredicate(uint32_t index, char op, T &target);

      ~SimpleDictPredicate() = default;

      void predicate(char op);

      shared_ptr<Bitmap> filterBlock(Block &, Bitmap &) override;
    };
    using SimpleInt32DictPredicate = SimpleDictPredicate<Int32Type>;
    using SimpleDoubleDictPredicate = SimpleDictPredicate<DoubleType>;
    using SimpleByteArrayDictPredicate = SimpleDictPredicate<ByteArrayType>;
